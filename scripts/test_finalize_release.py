...
class FinalizeReleaseTests(unittest.TestCase):
    ...
    def test_legacy_stable_preserves_exact_1_x_update_contract(self):
        candidate = build_candidate(
            self.create_github(), DEFAULT_GITHUB_REPOSITORY, "2.0.0"
        )
        payload = build_legacy_stable_payload(candidate, "2026-09-23T01:02:03Z")
        self.assertEqual(
            set(payload),
            {
                "schemaVersion",
        ...
        self.assertEqual(payload["minimumSupportedVersion"], "1.0.0")
        self.assertEqual(
            [(item["platform"], item["architecture"], item["installMode"]) for item in payload["packages"]],
            [
        ...
        self.assertTrue(payload["packages"][0]["fileName"].endswith(".exe"))
        self.assertTrue(all(item["size"] > 0 for item in payload["packages"]))
    ...
    def test_github_publish_failure_restores_gitee_prerelease(self):
        ...
        from finalize_release import publish_release_pair
    ...
    def test_entrypoint_switch_rolls_back_legacy_stable_if_freshness_fails(self):
        github = ContentChannel()
        gitee = ContentChannel()
        github.contents[ROOT_STABLE_ALIAS_PATH] = b"old-stable"
        gitee.contents[ROOT_STABLE_ALIAS_PATH] = b"old-stable"
        github.contents[ROOT_FRESHNESS_PATH] = b"old-freshness"
        gitee.contents[ROOT_FRESHNESS_PATH] = b"old-freshness"
        github.fail_write_once.add(ROOT_FRESHNESS_PATH)
        with self.assertRaisesRegex(RuntimeError, "write failed"):
            switch_release_entrypoints(
                github,
        ...
        self.assertEqual(github.read_content(ROOT_STABLE_ALIAS_PATH), b"old-stable")
        self.assertEqual(gitee.read_content(ROOT_STABLE_ALIAS_PATH), b"old-stable")
        self.assertEqual(github.read_content(ROOT_FRESHNESS_PATH), b"old-freshness")
        self.assertEqual(gitee.read_content(ROOT_FRESHNESS_PATH), b"old-freshness")

    def test_immutable_deploy_rolls_back_first_source(self):
        github = ContentChannel()
        gitee = ContentChannel()
        path = "releases/2.0.0/release.json"
    ...
    def test_finalize_and_refresh_preserve_immutable_documents(self):
        ...
        legacy = json.loads(github.contents[ROOT_STABLE_ALIAS_PATH])
        self.assertEqual(legacy["version"], "2.0.0")
        self.assertEqual(legacy["packages"][0]["installMode"], "replaceExecutable")
    ...
    def test_workflow_separates_secrets_and_schedules_four_hour_refresh(self):
        workflow = (
            Path(__file__).parents[1] / ".github/workflows/finalize-release.yml"
