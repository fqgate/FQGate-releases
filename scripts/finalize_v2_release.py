...
def validate_v2_version(value: str) -> tuple[int, int, int]:
    version = parse_version(value)
    if version[0] < 2:
        raise ValueError(f"V2 发布器只接受 2.0 及以上版本：{value}")
    return version
...
def read_active_state(
    ...
    stable_path = f"{V2_RELEASE_ROOT}/{freshness['stablePath']}"
    ...
    release_path = f"{V2_RELEASE_ROOT}/{stable['releasePath']}"
...
if __name__ == "__main__":
    try:
        ...
        print(f"FQGate 2.0 正式发布失败：{error}", file=sys.stderr)
        raise SystemExit(1)
